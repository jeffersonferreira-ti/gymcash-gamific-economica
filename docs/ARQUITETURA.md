...
**GymCash** · Documento Técnico · v1.2
...
### IdGenerator (`lib/utils/id_generator.dart`)

Gerador de IDs locais robusto. Combina timestamp em microsegundos com um contador atômico por processo.

**Problema resolvido:** `DateTime.now().microsecondsSinceEpoch.toString()` puro pode gerar IDs idênticos em operações muito rápidas executadas no mesmo microssegundo (ex: criar grupo + adicionar membro em sequência rápida).
...
|---|---|---|
| `UserModel` | `id`, `name` | `id` gerado via `IdGenerator`; serializado como objeto JSON único |
...
```
test/
├── widget_test.dart                      # Smoke test: renderização + validação de onboarding
├── models/
│   └── rank_model_test.dart              # RankModel e ContributionModel (11 casos)
└── services/
    ├── streak_service_test.dart          # StreakService (9 casos)
    └── local_storage_service_test.dart   # LocalStorageService (16 casos)
```

**Total: 51 casos — todos passando.**

Estratégia de teste para dados históricos: como `saveContribution` sempre usa o mês atual, testes que envolvem meses passados injetam dados diretamente via `SharedPreferences.setMockInitialValues`, serializando os modelos como JSON. Isso respeita o campo `month` sem contornar a lógica do service.
...
IdGenerator.newId() // → "1709123456789123_0"
```

IDs gerados localmente via `IdGenerator` (timestamp em microsegundos + contador atômico). Garante unicidade mesmo em operações rápidas consecutivas. Na migração para Firebase, o `id` local pode ser preservado ou substituído pelo `DocumentID` do Firestore.
...
```json
{
  "id":      "1709123456790000_1",
  "name":    "Turma de Investimentos",
  "members": [
    { "id": "1709123456789123_0", "name": "Jefferson" },
    { "id": "1709123456801000_2", "name": "Ana Silva" }
  ]
}
```
...
```json
{
  "id":              "1709123456810000_3",
  "userId":          "1709123456789123_0",
  "groupId":         "1709123456790000_1",
  "amount":          350.00,
  "goal":            500.00,
  "month":           "2025-03",
  "isGoalNotified":  true
...
```json
{
  "id":         "1709123456900000_4",
  "groupId":    "1709123456790000_1",
  "month":      "2025-02",
  "winnerId":   "1709123456789123_0",
  "winnerName": "Jefferson",
  "ranking": [
    { "userId": "1709123456789123_0", "userName": "Jefferson", "progress": 0.92, "position": 1 },
    { "userId": "1709123456801000_2", "userName": "Ana Silva",  "progress": 0.75, "position": 2 }
  ]
}
```
...
├── utils/
│   └── id_generator.dart             # gerador de IDs únicos (timestamp + contador)
├── screens/
...
test/
├── widget_test.dart                  # smoke test de inicialização
├── models/
│   └── rank_model_test.dart
└── services/
...
| IDs por `IdGenerator` | Firebase `DocumentID` (compatível — é string) |
